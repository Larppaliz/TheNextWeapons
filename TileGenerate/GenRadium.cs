...
{
	public class GenRadium : ModSystem
	{
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
		{
			...
			if (ShiniesIndex != -1)
			{
				...
				tasks.Insert(ShiniesIndex + 1, new ExampleOrePass("Radium", 237.4298f));
			}
		}
	}
	public class ExampleOrePass : GenPass
	{
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
		{
			if (ModContent.GetInstance<Config>().GenerateOres)
			{
				...
				progress.Message = "Rare Radioactive Ores";
				...
				for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 5E-05); k++)
				{
					...
					int y = WorldGen.genRand.Next(900, Main.maxTilesY);

					// Then, we call WorldGen.TileRunner with random "strength" and random "steps", as well as the Tile we wish to place.
					// Feel free to experiment with strength and step to see the shape they generate.
					WorldGen.TileRunner(x, y, WorldGen.genRand.Next(1, 7), WorldGen.genRand.Next(1, 8), ModContent.TileType<Tiles.RadiumOre>());
