...
namespace TheNextWeapons.TileGenerate {
public class GenSodalite : ModSystem

{
	public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
	{
		...
		if (ShiniesIndex != -1)
		{
			...
			tasks.Insert(ShiniesIndex + 1, new ExampleOrePass2("Sodalite", 237.4298f));
		}
	}
}
public class ExampleOrePass2 : GenPass
{
	public ExampleOrePass2(string name, float loadWeight) : base(name, loadWeight)
	{
	}
	...
	protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
	{
			if (ModContent.GetInstance<Config>().GenerateOres)
			{
				...
				progress.Message = "Rare Tasty...? Ores";
				...
				for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 6E-05); k++)
				{
					...
					int y = WorldGen.genRand.Next(500, Main.maxTilesY);

					// Then, we call WorldGen.TileRunner with random "strength" and random "steps", as well as the Tile we wish to place.
					// Feel free to experiment with strength and step to see the shape they generate.
					WorldGen.TileRunner(x, y, WorldGen.genRand.Next(3, 6), WorldGen.genRand.Next(2, 6), ModContent.TileType<Tiles.SodaliteOre>());
