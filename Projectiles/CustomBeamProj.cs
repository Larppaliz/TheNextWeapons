...
namespace TheNextWeapons.Projectiles
{
    public class CustomBeamProj : ModProjectile
    {
        ...
        private float _moveDist = 75f;       //The distance charge particle from the player center
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (_charge == 100)
            {
                ...
                unit.Normalize();
                DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center, unit, 5, projectile.damage, -1.57f, 1, 1000, Color.White, 75);
            }
            return false;
        ...
        /// </summary>
        public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 200f, Color color = default(Color), int transDist = 80)
        {
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                for (int k = 0; k < chargeFact + 1; k++)
                {
                    ...
                    Dust dust = Main.dust[Dust.NewDust(dustPos, 30, 30, 15, projectile.velocity.X / 2f,       //this 30, 30 is the dust weight and height 235 is the tail dust     
            ...
            for (int i = 0; i < 15; ++i)
            {
                ...
                Dust dust = Main.dust[Dust.NewDust(_targetPos, 0, 0, 15, dustVel.X, dustVel.Y, 0, new Color(), 1f)]; //this is the head dust
                Dust dust2 = Main.dust[Dust.NewDust(_targetPos, 0, 0, 15, dustVel.X, dustVel.Y, 0, new Color(), 1f)]; //this is the head dust 2
                dust.noGravity = true;
                dust.scale = 1.7f;
            }
